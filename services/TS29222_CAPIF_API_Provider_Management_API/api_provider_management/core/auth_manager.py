...
class AuthManager(Resource):
    ...
    def update_auth_provider(self, cert, func_id, provider_id):
        cert_col = self.db.get_col_by_name(self.db.certs_col)

        cert_encode = x509.load_pem_x509_certificate(str.encode(cert), default_backend())
        cert_col.find_one_and_update({"provider_id":func_id}, {"$set":{"cert_signature":cert_encode.signature.hex(), "id":func_id}})
