...
class InternalSecurityOps(Resource):
    ...
    def update_intern_servicesecurity(self, aef_id):
        ...
        security_contexts = security_col.find({"security_info.aef_id":aef_id})

        for security_context in security_contexts:
            new_security_info = [info for info  in security_context["security_info"] if info["aef_id"]!=aef_id]
