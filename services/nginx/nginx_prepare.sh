...
curl  -k -retry 30 \
    ...
    --retry-delay 10 \
    --retry-max-time 300 \
    --request GET "https://$EASY_RSA_HOSTNAME:$EASY_RSA_PORT/ca-root" 2>/dev/null | jq -r '.certificate' -j > $CERTS_FOLDER/ca.crt
...
curl -k --retry 30 \
    ...
    --retry-delay 10 \
    --retry-max-time 300 \
    --location --request POST "https://$EASY_RSA_HOSTNAME:$EASY_RSA_PORT/sign-csr" --header 'Content-Type: application/json' -d @./sign_req_body.json | jq -r '.certificate' -j > $CERTS_FOLDER/server.crt
